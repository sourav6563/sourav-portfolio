  ...
  return (
    <section id="featured" className="relative">
      <div className="container py-10 md:py-16 space-y-12">
        ...
        {/* Card */}
        <div className="mx-auto max-w-4xl rounded-2xl border border-white/8 bg-neutral-950 overflow-hidden">

          {/* Hero Image — Full Width */}
          <div className="relative w-full h-52 sm:h-64 md:h-72 overflow-hidden group">
            <Image
            ...
            <div className="absolute bottom-5 left-6 flex items-end gap-3">
              <h3 className="text-3xl font-bold text-white tracking-tight">dancely.in</h3>
            </div>
          </div>
          ...
          <div className="px-6 pt-5 pb-6 space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              {["TypeScript", "Express.js", "MongoDB", "Zod", "Next.js", "React", "Tailwind CSS", "Shadcn UI", "Cloudinary", "Resend"].map((tech) => (
                <Badge key={tech} variant="secondary" className="bg-neutral-900 text-neutral-300 border-neutral-800 hover:bg-neutral-800">
                  {tech}
                </Badge>
              ))}
            </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                {links.map((link) => (
                  <Button
                    ...
                    className={
                      link.primary
                        ? "rounded-full bg-white text-black hover:bg-neutral-200 transition-all duration-200 cursor-pointer"
                        : "rounded-full border-white/12 text-neutral-400 hover:bg-white/8 hover:border-white/20 hover:text-white transition-all duration-200 cursor-pointer"
                    }
                  >
                    <a href={link.href} target="_blank" rel="noreferrer" className="flex items-center">
                      <link.icon className="mr-1.5 h-3.5 w-3.5" /> {link.label}
                    </a>
                  </Button>
                ))}
                ...
                <a href="https://github.com/sourav6563/dancely-backend" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs text-neutral-400 hover:text-white transition-colors px-2.5 py-1.5 rounded-full hover:bg-white/5">
                  <Github className="h-3.5 w-3.5" /> Backend
                </a>
                <a href="https://github.com/sourav6563/dancely-frontend" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs text-neutral-400 hover:text-white transition-colors px-2.5 py-1.5 rounded-full hover:bg-white/5">
          ...
          <div className="grid md:grid-cols-2 border-t border-white/5">
            ...
            {/* Backend Column */}
            <div className="p-6 md:border-r border-white/5">
              <p className="text-xs font-semibold tracking-widest text-neutral-500 uppercase mb-4">Backend Highlights</p>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                {backendHighlights.map((item, i) => (
                  <li key={i} className="flex gap-2 text-xs leading-normal">
            ...
            {/* Frontend Column */}
            <div className="p-6 border-t md:border-t-0 border-white/5">
              <p className="text-xs font-semibold tracking-widest text-neutral-500 uppercase mb-4">Frontend Highlights</p>
              <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
